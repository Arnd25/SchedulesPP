...
export const UsersList = async () => {
    ...
    return (
        <div className="px-6 py-6 bg-secondary rounded-lg flex flex-col gap-2.5 h-full overflow-hidden">
            <h2 className='text-4xl shrink-0'>Пользователи</h2>

            <div className="overflow-hidden">
                <ul className='flex flex-col gap-2.5 overflow-y-auto h-full sidebar-scroll'>
                    {data.map((item) => (
                        <li key={item.id} className="p-5 bg-card flex gap-2.5 items-center rounded-lg text-2xl font-semibold shrink-0">
                            <UserCard data={item} />
                        </li>
                    ))}
                </ul>
            ...
            <Link className='w-full flex bg-primary rounded-lg py-2.5 text-white text-xl justify-center shrink-0' href={APP_ROUTES.users()}>
                Подробнее
            </Link>
        </div>
    )
